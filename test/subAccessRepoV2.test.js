...
test('sub access repo counts distinct UAs case-insensitively and clears recent window', async (t) => {
  const dir = await fsp.mkdtemp(path.join(os.tmpdir(), 'vless-sub-ua-window-'));
  const dbPath = path.join(dir, 'panel.db');
  const db = new Database(dbPath);
  ...
  db.prepare(`
    INSERT INTO sub_access_log (user_id, ip, ua, created_at) VALUES
    (1, '1.1.1.1', 'Clash', datetime('now')),
    (1, '1.1.1.2', ' clash ', datetime('now')),
    (1, '1.1.1.3', 'v2rayN/6.45', datetime('now')),
    (1, '1.1.1.4', '', datetime('now')),
    (1, '1.1.1.5', 'Shadowrocket', datetime('now', '-30 hours'))
  `).run();
  db.prepare(`
    INSERT INTO sub_access_event (user_id, token_prefix, route, result, reason, ip, ua, client_type, http_status, created_at) VALUES
    (1, 'tok-a', 'sub', 'allow', 'ok', '1.1.1.1', 'Clash', 'clash', 200, datetime('now')),
    (1, 'tok-a', 'sub', 'allow', 'ok', '1.1.1.2', 'v2rayN/6.45', 'v2ray', 200, datetime('now')),
    (1, 'tok-a', 'sub', 'allow', 'ok', '1.1.1.3', 'Shadowrocket', 'clash', 200, datetime('now', '-30 hours'))
  `).run();

  assert.equal(subAccessRepo.countDistinctSubAccessUas(1, 24), 2);

  subAccessRepo.clearSubAccessWindow(1, 24);

  const recentLogCount = db.prepare(`
    SELECT COUNT(*) AS c FROM sub_access_log
    WHERE user_id = 1 AND created_at > datetime('now', '-24 hours')
  `).get().c;
  const recentEventCount = db.prepare(`
    SELECT COUNT(*) AS c FROM sub_access_event
    WHERE user_id = 1 AND created_at > datetime('now', '-24 hours')
  `).get().c;
  const oldLogCount = db.prepare(`
    SELECT COUNT(*) AS c FROM sub_access_log
    WHERE user_id = 1 AND created_at <= datetime('now', '-24 hours')
  `).get().c;

  assert.equal(recentLogCount, 0);
  assert.equal(recentEventCount, 0);
  assert.equal(oldLogCount, 1);
});
