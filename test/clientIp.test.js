...
test('getClientIp prefers Cloudflare client ip when cf headers are present', () => {
  const req = {
    ip: '172.69.176.158',
    headers: {
      'cf-connecting-ip': '203.0.113.77',
      'cf-ray': 'abcd-SJC',
      'x-forwarded-for': '172.69.176.158',
    },
  };
  assert.equal(getClientIp(req), '203.0.113.77');
});

test('getClientIp ignores cf-connecting-ip without Cloudflare signal on public direct connection', () => {
  const req = {
