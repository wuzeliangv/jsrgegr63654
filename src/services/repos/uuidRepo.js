...
function rotateUserAllNodeUuids(userId) {
  const rows = _getDb().prepare('SELECT id FROM user_node_uuid WHERE user_id = ?').all(userId);
  const stmt = _getDb().prepare('UPDATE user_node_uuid SET uuid = ? WHERE id = ?');
  const updateMany = _getDb().transaction((items) => {
...
function rotateUserNodeUuidsByNodeIdsAndLevels(nodeIds = [], levels = []) {
  if (!nodeIds.length || !levels.length) return 0;
  const np = nodeIds.map(() => '?').join(',');
  const lp = levels.map(() => '?').join(',');
  const rows = _getDb().prepare(
    `SELECT un.id FROM user_node_uuid un JOIN users u ON un.user_id = u.id WHERE un.node_id IN (${np}) AND u.trust_level IN (${lp})`
  ).all(...nodeIds, ...levels);
  const stmt = _getDb().prepare('UPDATE user_node_uuid SET uuid = ? WHERE id = ?');
  _getDb().transaction(() => { for (const r of rows) stmt.run(uuidv4(), r.id); })();
  return rows.length;
}
...
module.exports = {
  ...
  rotateAllUserNodeUuids, rotateUserAllNodeUuids, rotateUserNodeUuidsByNodeIds, rotateUserNodeUuidsByNodeIdsAndLevels
};
