...
function countDistinctSubAccessUas(userId, hours = 24) {
  // 优先从 sub_access_event 统计（包含被拒绝的请求），回退到 sub_access_log
  const tbl = (() => { try { _getDb().prepare('SELECT 1 FROM sub_access_event LIMIT 0').run(); return 'sub_access_event'; } catch (_) { return 'sub_access_log'; } })();
  const row = _getDb().prepare(`
    SELECT COUNT(*) as ua_count
    FROM (
      SELECT LOWER(TRIM(ua)) as ua_key
      FROM ${tbl}
      WHERE user_id = ?
        AND created_at > datetime('now', '-' || ? || ' hours')
        AND TRIM(COALESCE(ua, '')) <> ''
      GROUP BY ua_key
    )
  `).get(userId, hours);
  return Number(row?.ua_count || 0);
}

function clearSubAccessWindow(userId, hours = 24) {
  const safeHours = toPosInt(hours, 24, 1, 24 * 365);
  const tx = _getDb().transaction(() => {
    _getDb().prepare(`
      DELETE FROM sub_access_log
      WHERE user_id = ?
        AND created_at > datetime('now', '-' || ? || ' hours')
    `).run(userId, safeHours);
    _getDb().prepare(`
      DELETE FROM sub_access_event
      WHERE user_id = ?
        AND created_at > datetime('now', '-' || ? || ' hours')
    `).run(userId, safeHours);
  });
...
function logSubAccessEvent(input = {}) {
  ...
  const parsedUserId = (rawUserId === null || rawUserId === undefined || rawUserId === '')
    ? NaN
    : Number(rawUserId);
